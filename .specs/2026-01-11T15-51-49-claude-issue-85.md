...
Payment Success → Redirect to /success?session_id=XXX
                ↓ (immediate)
              Poll API with exponential backoff (max 5 attempts)
                ↓
              Query DB for payment_intent_id
                ↓
...
const MAX_RETRY_ATTEMPTS = 5; // Max 5 attempts = ~31 seconds total
const BACKOFF_MULTIPLIER = 2; // Exponential backoff factor (industry standard)
...
A comprehensive manual testing checklist has been created for human testers.

**Checklist Document**: `.specs/MANUAL_TESTING_CHECKLIST_ISSUE_85.md`
...
2. Complete checkout
3. Observe success page shows "Loading order details..." for ~31 seconds
...
**Additional Test Cases in Checklist**:

- Test Case 5: Network error during polling
- Test Case 6: Browser navigation during polling (cleanup verification)
- Test Case 7: Multiple artworks in cart
- Test Case 8: Rapid page reload during polling
- Edge cases: Browser compatibility, accessibility, performance metrics

**Checklist Features**:

- Step-by-step instructions with checkboxes
- Expected results for each scenario
- Setup requirements and test card numbers
- Debugging tips and troubleshooting guide
- Sign-off section for tester accountability
- Success criteria matrix (critical vs. recommended tests)

**Human Action Required**: Review and execute the manual testing checklist before PR approval.
...
- ✅ New `useOrderPolling` hook with retry logic (5 attempts, ~31s total)
...
Polling tries up to 5 times over ~31 seconds with exponential backoff (2x multiplier),
giving webhook time to complete. Degrades gracefully with user-friendly message
if order creation truly fails.
```
...
- **Client-side polling**: ~5 API calls per successful checkout (acceptable low-traffic scenario)
- **Exponential backoff**: Reduces server load over time with 2x multiplier (1s, 2s, 4s, 8s, 16s delays)
...
1. **Polling parameters**: ✅ **RESOLVED** - Using 5 retries over ~31 seconds
    - **Selected**: 5 retries, 1s initial, 2x backoff (industry standard exponential backoff)
    - **Rationale**: Standard 2x multiplier is well-understood, ~31s gives ample time for webhook completion
    - **Alternative considered**: 1.5x multiplier (~13s total) - rejected as potentially too aggressive
...
2. **Secondary Metrics**:
    ...
    - Polling timeout rate: Target <1% (webhook completes within 31s)
...
| Polling increases API load           | Medium | Low         | Exponential backoff (2x), max 5 retries                 |
