...
**Date:** 2025-11-06 **Status:** Planning **Parent Spec:**
[MVP Implementation Plan](./2025-10-25T17-55-00-mvp-implementation-plan.md)

---

## Overview

Phase 3 focuses on building a complete admin system for managing site content,
orders, and media. This includes authentication (already implemented), a
dashboard, content management for artwork/projects/events, order management, and
image upload capabilities.

### Relationship to Full MVP Plan

This specification extracts and expands Phase 3 from the
[MVP Implementation Plan](./2025-10-25T17-55-00-mvp-implementation-plan.md). It
integrates testing requirements directly into implementation steps, following
...
**Critical Rule**: No feature implementation without a failing test first. Where
existing testing gaps are discovered, add tests for those features before
proceeding.

### Prerequisites (Completed in Earlier Phases)

- ✅ Phase 1: Database schema with `artwork`, `orders`, `projects`, `events`,
...
### Prerequisites (Must Be Configured Before Starting Phase 3) ✅ Complete

#### Supabase Storage Setup ✅ Complete

Before beginning Phase 3 implementation, configure Supabase Storage for image
uploads:
...
The proxy (formerly middleware) currently stores `userId`, `adminId`, `role`,
and `expiresAt` in the session cookie, but the admin header needs to display the
admin's name. Update the proxy:
...
**Rationale**: The admin header component needs to display the admin's name
without making additional database queries. Including it in the cached session
cookie provides this data efficiently.
...
3. **Image Upload**: Upload and optimize images with variants (thumbnail,
   preview, large)
...
- Session validation checks user in `administrators` table with
  `is_active = true`
- Redirects to `/admin/login` if authentication fails

**Database Query Patterns**: All existing query functions
(`src/lib/db/artwork.ts`, `events.ts`, `projects.ts`, `pages.ts`) follow this
pattern:
...
- Variants and sizes via props (e.g., Button:
...
- **Browser client** (`src/lib/supabase/client.ts`): Uses public anon key for
  public queries and auth
- **Server client** (`src/lib/supabase/server.ts`): Uses service role key for
  admin operations and RLS bypass
...
- TypeScript and Jest configured with: `@/`, `@/app/`, `@/components/`,
  `@/lib/`, `@/types/`, `@/hooks/`, `@/context/`
...
- AdminNavigation - Responsive navigation (menu bar on desktop, sidebar on
  mobile)
...
**Decision**: Admin operations will use the Supabase service role key to bypass
RLS policies, rather than extending RLS policies to handle admin access.

**Reasoning**:

This project uses **service role key approach** for admin operations for these
reasons:

1. **Simpler RLS policies**: Public RLS policies remain focused on public access
   only (`is_published = true` filters). No need to handle admin role checks in
   every RLS policy.

2. **Centralized authorization**: All admin authorization happens in one place
   (proxy), not scattered across multiple RLS policies per table. Easier to
   audit and maintain.

3. **Development velocity**: MVP phase prioritizes speed. Simple RLS policies =
   faster development. Complex role-based RLS can be added later if needed.

4. **Performance**: No RLS policy evaluation overhead on admin operations.
   Important for content updates and bulk operations.

5. **Flexibility**: Can perform operations that don't fit RLS model (cross-table
   queries, bulk updates, admin-specific logic).

6. **Clear separation**: Public queries use anon key
   (`src/lib/supabase/client.ts`), admin queries use service role
   (`src/lib/supabase/server.ts`). Easy to audit which code paths have elevated
   privileges.

**Security Safeguards**:

While the service role key has full database access, we mitigate risks with
these safeguards:
...
2. **proxy protection**: All admin routes protected by proxy that validates
   admin session before allowing access

3. **API route protection**: All admin mutations go through `/api/admin/*`
   routes that verify session. Even if service key leaked, attacker needs valid
   admin session cookie.

4. **Server-side only enforcement**: Admin query functions include runtime
   checks to ensure they're only called server-side:
...
5. **Application-level logging**: All admin operations logged in application
   code for audit trail

6. **Defense in depth**: proxy + API route checks + server-side enforcement =
   multiple layers of protection

**Alternative Considered**: Extending RLS policies to handle admin roles was
considered but rejected due to:

- Complexity: Every table would need admin role checks in RLS policies
- Performance: RLS evaluation overhead on every admin query (JOIN to
...
**Future Migration Path**: If more granular permissions are needed (e.g.,
"editor" role that can update but not delete), RLS-based admin access can be
added without changing public-facing code. The service role approach provides
velocity now with acceptable risk given our safeguards.
...
**Decision**: Admin pages will have a dedicated layout that replaces the public
navigation with an admin-specific navigation system.
...
- **Logo**: Small version of site logo in top-left corner (links to `/admin`
  dashboard)
...
- **Desktop** (≥1024px): Horizontal menu bar with links:
    - Dashboard | Artwork | Orders | Projects | Events | Settings (super_admin
      only)
...
4. **Clean separation**: Admin navigation completely separate from public
   navigation
...
**Decision**: All admin form submissions use Next.js Server Actions, not API
routes.

**Pattern**:

Admin query functions (`src/lib/db/admin/*.ts`) use the service role client and
must run server-side only. To call these from client components (forms), use
Server Actions:
...
- **Use API Routes**: For file uploads (`/api/admin/upload`), webhooks, or when
  non-Next.js clients need access

### 8. Authentication Patterns

**Critical**: proxy protects `/admin/*` page routes but **does NOT protect
`/api/admin/*` API routes**. All API routes must manually verify authentication.
...
For UI-only decisions (e.g., showing/hiding Settings link), read cookie
client-side:
...
**Security Note**: Client-side cookie checks are for UI convenience only. Always
verify authorization server-side (proxy, Server Actions, or API routes).
...
**Consistent Naming**: Always use database field names throughout the
application. The "preview" size is stored in the `image_url` field.
...
To ensure code quality and prevent technical debt accumulation, run the
verification procedure after completing each TDD step (Green and Refactor) and
at the end of each phase.

### When to Run Verification

- **After TDD Green Step**: Run verification after implementing minimal code to
  make tests pass
- **After TDD Refactor Step**: Run verification after refactoring to ensure
  improvements don't break functionality
- **After Each Phase**: Run full verification before moving to the next phase
- **Note**: During TDD Red step, newly-added tests are expected to fail, so
  verification is not required

### Verification Commands

Run these commands in order, replacing `{files}` with space-separated paths of
files modified in the current step/phase:

1. **TypeScript Compile Check** (run as-is, no file paths needed):

    ```bash
    npx tsc --noEmit
    ```

2. **ESLint** (auto-fix linting issues):
...
4. **Vitest** (run tests related to modified files):
    ```bash
...
1. **Identify Modified Files**: List all file paths modified in the current TDD
   step or phase
    - Example:
      `src/lib/db/admin/dashboard.ts src/components/admin/AdminCard.tsx __tests__/lib/db/admin/dashboard.test.ts`
...
2. **Run TypeScript Compile Check**:
    ...
    - This checks the entire codebase for type errors
    - Fix any type errors before proceeding

3. **Run ESLint**:

    ```bash
    npx eslint --fix src/lib/db/admin/dashboard.ts src/components/admin/AdminCard.tsx
    ```

    - Replace with your actual file paths
    - ESLint will auto-fix many issues
    - Manually fix any remaining linting errors
    - Do NOT disable any rules except in specific cases where the implementation
      strategy absolutely requires it.
    - If you must disable a rule, document the reason in comments.

4. **Run Prettier**:

    ```bash
    npx prettier --write src/lib/db/admin/dashboard.ts src/components/admin/AdminCard.tsx __tests__/lib/db/admin/dashboard.test.ts
    ```

    - Replace with your actual file paths (including test files)
    - Prettier will format all files automatically

5. **Run Tests**:

    ```bash
    npx vitest related run src/lib/db/admin/dashboard.ts src/components/admin/AdminCard.tsx
    ```

    - This runs tests related to the modified source files
    - During TDD Green: All tests should pass (including newly added tests)
    - During TDD Refactor: All tests should still pass
...
7. **Success Criteria**:
    - All TypeScript checks pass (no type errors)
    - All ESLint checks pass (no errors or warnings)
    - All files properly formatted by Prettier
    - All tests pass (green)
    - Only proceed to the next step/phase when ALL verifications pass

### Example Verification Workflow

**Scenario**: You just completed Phase 3.1.2 (TDD Green - Implement Dashboard
Metrics Query)

**Modified Files**:

- `src/lib/db/admin/dashboard.ts` (new file)
- `__tests__/lib/db/admin/dashboard.test.ts` (existing file with new tests)

**Run Verification**:

```bash
# 1. TypeScript check (entire codebase)
npx tsc --noEmit

# 2. Lint the source file
npx eslint --fix src/lib/db/admin/dashboard.ts

# 3. Format all modified files
npx prettier --write src/lib/db/admin/dashboard.ts __tests__/lib/db/admin/dashboard.test.ts

# 4. Run related tests
npx vitest related run src/lib/db/admin/dashboard.ts
```

**Expected Result**: All commands succeed, all tests pass (green). Proceed to
next task (3.1.3).

### Verification Frequency Summary

| TDD Phase                     | Verification Required | Notes                                         |
| ----------------------------- | --------------------- | --------------------------------------------- |
| **Red** (Write failing tests) | ❌ No                 | Tests are expected to fail                    |
| **Green** (Implement code)    | ✅ Yes                | Run verification after implementation         |
| **Refactor** (Improve code)   | ✅ Yes                | Run verification after refactoring            |
| **End of Phase**              | ✅ Yes                | Full verification before moving to next phase |
...
[Code Verification Procedure](#code-verification-procedure) after each TDD
Green/Refactor step and at the end of each phase. This ensures code quality and
prevents technical debt accumulation.

### Phase 3.1: Admin Dashboard & Metrics ✅ Complete
...
- [x] Create `__tests__/lib/db/admin/dashboard.test.ts`
- [x] Write failing tests for `getDashboardMetrics()`:
    - [x] Test returns total orders count
    - [x] Test returns orders this month count
    - [x] Test returns total revenue
    - [x] Test returns pending orders count
    - [x] Test uses service role client
    - [x] Test handles database errors gracefully
    - [x] Test returns { data, error } format
...
- [x] Create `src/lib/db/admin/dashboard.ts`
- [x] Implement `getDashboardMetrics()`:
    - [x] Query orders table for counts and sums
    - [x] Use server client (service role key)
    - [x] Follow { data, error } return pattern
    - [x] Calculate metrics:
        - [x] Total orders: COUNT(\*)
        - [x] Orders this month: COUNT(\*) WHERE created_at >= start of month
        - [x] Total revenue: SUM(total_amount)
        - [x] Pending orders: COUNT(\*) WHERE status = 'pending'
    - [x] Handle errors with proper error interface
- [x] Run tests - should now pass
- [x] **Run Verification Procedure** (see
      [Code Verification Procedure](#code-verification-procedure)):
    - [x] Modified files: `src/lib/db/admin/dashboard.ts`,
          `__tests__/lib/db/admin/dashboard.test.ts`
    - [x] Run: `npx tsc --noEmit`
    - [x] Run: `npx eslint --fix src/lib/db/admin/dashboard.ts`
    - [x] Run:
          `npx prettier --write src/lib/db/admin/dashboard.ts __tests__/lib/db/admin/dashboard.test.ts`
    - [x] Run: `npx vitest related run src/lib/db/admin/dashboard.ts`
    - [x] All verifications must pass before proceeding

#### 3.1.3: Write Tests for Recent Orders Query

**TDD Step: Red**

- [x] Add tests to `__tests__/lib/db/admin/dashboard.test.ts` for
      `getRecentOrders()`:
    - [x] Test returns last 10 orders
    - [x] Test orders sorted by created_at DESC
    - [x] Test includes customer name and email
    - [x] Test includes order total and status
    - [x] Test pagination works (limit parameter)
...
- [x] Add `getRecentOrders()` to `src/lib/db/admin/dashboard.ts`:
    - [x] Query orders table with limit (default 10)
    - [x] Sort by created_at DESC
    - [x] Select relevant fields (order_number, customer_name, total_amount,
          status, created_at)
    - [x] Return { data, error } format
- [x] Run tests - should pass
- [x] **Run Verification Procedure**:
    - [x] Modified files: `src/lib/db/admin/dashboard.ts`,
          `__tests__/lib/db/admin/dashboard.test.ts`
    - [x] Run:
          `npx tsc --noEmit && npx eslint --fix src/lib/db/admin/dashboard.ts && npx prettier --write src/lib/db/admin/dashboard.ts __tests__/lib/db/admin/dashboard.test.ts && npx vitest related run src/lib/db/admin/dashboard.ts`

#### 3.1.5: Write Tests for AdminCard Component

**TDD Step: Red**

- [x] Create `__tests__/components/admin/AdminCard.test.tsx`
- [x] Write tests:
    - [x] Renders title prop
    - [x] Renders children content
    - [x] Applies custom className
    - [x] Shows loading state (optional skeleton)
    - [x] Renders metric value with label
...
- [x] Create `src/components/admin/AdminCard.tsx`
- [x] Extend existing Card component pattern
- [x] Props:
    - [x] `title: string`
    - [x] `value?: string | number` (for metrics)
    - [x] `label?: string` (for metric description)
    - [x] `loading?: boolean`
...
- [x] Run tests - should pass
- [x] **Run Verification Procedure**:
    - [x] Modified files: `src/components/admin/AdminCard.tsx`,
          `__tests__/components/admin/AdminCard.test.tsx`
    - [x] Run:
          `npx tsc --noEmit && npx eslint --fix src/components/admin/AdminCard.tsx && npx prettier --write src/components/admin/AdminCard.tsx __tests__/components/admin/AdminCard.test.tsx && npx vitest related run src/components/admin/AdminCard.tsx`

#### 3.1.7: Write Tests for AdminHeader Component

**TDD Step: Red**

- [x] Create `__tests__/components/admin/AdminHeader.test.tsx`
- [x] Write tests:
    - [x] Renders small logo in top-left
    - [x] Logo links to `/admin` dashboard
    - [x] Displays current admin name
    - [x] Renders logout button
    - [x] Logout button calls signOut()
    - [x] Compact height (~60px)
    - [x] On mobile: renders hamburger menu button
    - [x] Hamburger button toggles navigation state
...
- [x] Client component (needs state for mobile menu, useAuth for logout)
- [x] Props:
    - [x] `adminName: string`
    - [x] `onMenuToggle?: () => void` (for mobile sidebar toggle)
- [x] Layout structure:
    - [x] Container with ~60px height
    - [x] Left: Small logo (link to `/admin`)
    - [x] Center: Admin name display (visible on desktop, hidden on mobile)
    - [x] Right: Logout button
    - [x] Mobile: Hamburger menu button (left side, before logo)
- [x] Use existing Button component for logout
- [x] Style with Tailwind (black/white theme)
- [x] Run tests - should pass
- [x] **Run Verification Procedure**:
    - [x] Modified files: `src/components/admin/AdminHeader.tsx`,
          `__tests__/components/admin/AdminHeader.test.tsx`
    - [x] Run verification commands for modified files

#### 3.1.9: Write Tests for AdminNavigation Component

**TDD Step: Red**

- [x] Create `__tests__/components/admin/AdminNavigation.test.tsx`
- [x] Write tests:
    - [x] Desktop (≥1024px): Renders horizontal menu bar
    - [x] Desktop: Shows all navigation links (Dashboard, Artwork, Orders,
          Projects, Events)
    - [x] Desktop: Shows Settings link only for super_admin role
    - [x] Desktop: Highlights active route
    - [x] Mobile (<1024px): Renders as sidebar
    - [x] Mobile: Sidebar hidden by default (isOpen = false)
    - [x] Mobile: Sidebar visible when isOpen = true
    - [x] Mobile: Overlay backdrop present when open
    - [x] Mobile: Clicking backdrop closes sidebar
    - [x] Mobile: Navigation links stacked vertically
    - [x] All screen sizes: Links navigate to correct routes

#### 3.1.10: Implement AdminNavigation Component

**TDD Step: Green**

- [x] Create `src/components/admin/AdminNavigation.tsx`
- [x] Client component (needs usePathname for active route)
- [x] Props:
    - [x] `role: 'admin' | 'super_admin'`
    - [x] `isOpen: boolean` (for mobile sidebar state)
    - [x] `onClose: () => void` (callback to close mobile sidebar)
- [x] Navigation links array:
    ...
    ```
- [x] Desktop rendering (Tailwind `hidden lg:flex`):
    - [x] Horizontal flex container (~48px height)
    - [x] Links with padding and hover states
    - [x] Active link with underline or background
    - [x] Filter out Settings if role !== 'super_admin'
- [x] Mobile rendering (Tailwind `lg:hidden`):
    - [x] Sidebar container (256px width, fixed position)
    - [x] Slide-in animation (translate-x-0 when open)
    - [x] Backdrop overlay (click to close)
    - [x] Vertical stacked links
    - [x] Close button or swipe gesture
- [x] Run tests - should pass
...
- [x] Create `__tests__/app/admin/layout.test.tsx`
- [x] Write tests:
    - [x] Renders AdminHeader
    - [x] Renders AdminNavigation
    - [x] Renders children content
    - [x] Has proper semantic structure (header, nav, main)
    - [x] Passes admin name to header
    - [x] Passes role to navigation
    - [x] Mobile menu toggle state works
...
- [x] Client component (needs state for mobile menu)
- [x] Read admin name and role from admin_session cookie
- [x] State: `const [mobileMenuOpen, setMobileMenuOpen] = useState(false)`
- [x] Structure:
    ```tsx
    <div className="admin-layout">
        <AdminHeader
...
- [x] Create `__tests__/app/admin/dashboard.test.tsx`
- [x] Write tests:
    - [x] Renders dashboard title
    - [x] Displays all metric cards (4 metrics)
    - [x] Shows recent orders list
    - [x] Handles loading state
    - [x] Handles error state (metrics query fails)
    - [x] Displays order details (number, customer, total, status)
    - [x] Links to order detail pages

#### 3.1.14: Implement Dashboard Page

**TDD Step: Green**

- [x] Update `src/app/admin/page.tsx` (currently placeholder)
- [x] Server component
- [x] Fetch dashboard metrics using `getDashboardMetrics()`
- [x] Fetch recent orders using `getRecentOrders()`
- [x] Handle errors gracefully (show error message)
- [x] Layout:
    - [x] Page title: "Dashboard"
    - [x] Metrics grid (4 AdminCard components):
        - [x] Total Orders
        - [x] Orders This Month
        - [x] Total Revenue
        - [x] Pending Orders
    - [x] Recent orders section:
        - [x] Table with columns: Order Number, Customer, Total, Status, Date
        - [x] Link to order detail page
        - [x] Empty state message if no orders
- [x] Run tests - should pass
...
- [x] **Run Verification Procedure** after refactoring:
    - [x] List all files modified during refactoring
    - [x] Run: `npx tsc --noEmit`
    - [x] Run: `npx eslint --fix {refactored-files}`
    - [x] Run: `npx prettier --write {all-phase-3.1-files}`
    - [x] Run: `npx vitest related run {refactored-files}`
- [x] Run full test suite: `npm test`
- [x] Verify coverage: `npm run test:coverage`
- [x] **End-of-Phase Verification** (comprehensive check):
    - [x] Identify ALL files created/modified in Phase 3.1:
        - Dashboard queries (`src/lib/db/admin/dashboard.ts`)
        - Admin components (`src/components/admin/AdminCard.tsx`, `AdminHeader.tsx`,
          `AdminNavigation.tsx`)
    ...
    - [x] Run ESLint:
          `npx eslint --fix src/lib/db/admin/ src/components/admin/ src/app/admin/`
    - [x] Run Prettier:
          `npx prettier --write src/lib/db/admin/ src/components/admin/ src/app/admin/ __tests__/`
    - [x] Run all Phase 3.1 tests:
          `npm test -- __tests__/lib/db/admin/dashboard.test.ts __tests__/components/admin/ __tests__/app/admin/`
    - [x] Verify all checks pass before proceeding to Phase 3.2
- [x] Manual testing:
    - [x] Navigate to `/admin`
    - [x] Verify metrics display (may be 0 if no orders)
    - [x] Desktop: Verify horizontal menu bar displays below header
    - [x] Desktop: Click each navigation link (Dashboard, Artwork, Orders, etc.)
    - [x] Desktop: Verify active route is highlighted
    - [x] Mobile: Verify hamburger menu button appears
    - [x] Mobile: Click hamburger to open sidebar
    - [x] Mobile: Verify sidebar slides in from left
    - [x] Mobile: Click backdrop to close sidebar
    - [x] Mobile: Verify navigation links work in sidebar
    - [x] Test logout functionality
    - [x] Verify logo links to dashboard
...
- [x] Create `__tests__/lib/db/admin/artwork.test.ts`
- [x] Write failing tests for:
    - [x] `getAllArtworkAdmin()` - returns ALL artwork (not just published)
    - [x] `getArtworkById(id)` - returns single artwork by UUID
    - [x] `createArtwork(data)` - inserts new artwork
    - [x] `updateArtwork(id, data)` - updates existing artwork
    - [x] `deleteArtwork(id)` - deletes artwork
    - [x] Test all functions use server client
    - [x] Test { data, error } return pattern
    - [x] Test error handling (missing ID, invalid data, DB errors)

#### 3.2.2: Implement Admin Artwork Queries

**TDD Step: Green**

- [x] Create `src/lib/db/admin/artwork.ts`
- [x] Implement all query functions:
    - [x] `getAllArtworkAdmin(limit?, offset?)` - no is_published filter
    - [x] `getArtworkById(id: string)` - single select by UUID
    - [x] `createArtwork(data: ArtworkInput)` - insert with validation
    - [x] `updateArtwork(id: string, data: Partial<ArtworkInput>)` - update
    - [x] `deleteArtwork(id: string)` - delete by UUID
- [x] Use server client (service role)
- [x] Define `ArtworkInput` type (extend database type)
- [x] Define `ArtworkAdminError` interface
- [x] Run tests - should pass
- [x] **Run Verification Procedure**:
    - [x] Modified files: `src/lib/db/admin/artwork.ts`,
          `__tests__/lib/db/admin/artwork.test.ts`
    - [x] Run verification commands for these files

#### 3.2.3: Write Tests for ArtworkForm Component

**TDD Step: Red**

- [x] Create `__tests__/components/admin/ArtworkForm.test.tsx`
- [x] Write tests:
    - [x] Renders all form fields (title, description, slug, price, etc.)
    - [x] Validates required fields (title, slug)
    - [x] Shows validation errors
    - [x] Submit button disabled when invalid
    - [x] Calls onSubmit with form data
    - [x] Pre-fills form in edit mode
    - [x] Image upload integration (ImageUploader component)
...
- [x] Create `src/lib/validation/artwork.ts`
- [x] Define Zod schema for artwork:
    ```typescript
    ...
    ```
- [x] Export `ArtworkFormData` type from schema

#### 3.2.5: Implement ArtworkForm Component

**TDD Step: Green**

- [x] Create `src/components/admin/ArtworkForm.tsx`
- [x] Client component (form interactions)
- [x] Props:
    - [x] `initialData?: ArtworkFormData` (for edit mode)
    - [x] `onSubmit: (data: ArtworkFormData) => Promise<void>`
    - [x] `isLoading?: boolean`
- [x] Form fields (use Input, Textarea components):
    - [x] Title (text, required)
    - [x] Slug (text, required, pattern validation)
    - [x] Description (textarea)
    - [x] Price (number)
    - [x] Original Price (number)
    - [x] SKU (text)
    - [x] Inventory Count (number, default 0)
    - [x] Is Limited Edition (checkbox)
    - [x] Medium (text)
    - [x] Dimensions (text)
    - [x] Year Created (number)
    - [x] Is Published (checkbox)
    - [x] Is Featured (checkbox)
    - [x] Display Order (number)
    - [x] Image Upload (ImageUploader component - TBD)
    - [x] Alt Text (text)
    - [x] SEO Title (text)
    - [x] SEO Description (textarea)
    - [x] Tags (text input with comma separation)
- [x] Use Zod validation with artworkSchema
- [x] Show validation errors inline
- [x] Submit button with loading state
- [x] Cancel button
- [x] Run tests - should pass
- [x] **Run Verification Procedure**:
    - [x] Modified files: `src/components/admin/ArtworkForm.tsx`,
          `src/lib/validation/artwork.ts`,
          `__tests__/components/admin/ArtworkForm.test.tsx`
    - [x] Run verification commands for these files
...
- [x] Create `__tests__/app/admin/artwork/page.test.tsx`
- [x] Write tests:
    - [x] Renders page title "Artwork Management"
    - [x] Shows "Add New Artwork" button
    - [x] Displays artwork in table/grid
    - [x] Table columns: Thumbnail, Title, Price, Status, Actions
    - [x] Edit button links to edit page
    - [x] Delete button shows confirmation
    - [x] Shows empty state if no artwork
    - [x] Handles loading state
...
- [x] Create `src/app/admin/artwork/page.tsx`
- [x] Server component
- [x] Fetch artwork using `getAllArtworkAdmin()`
- [x] Layout:
    - [x] Page header with title "Artwork Management"
    - [x] "Add New Artwork" button (links to `/admin/artwork/new`)
    - [x] Table/grid of artwork:
        - [x] Thumbnail image (if available)
        - [x] Title
        - [x] Price (or "N/A")
        - [x] Status badge (Published/Draft)
        - [x] Actions: Edit button, Delete button
...
- [ ] Review code quality:
    ...
    - [ ] Add JSDoc comments
    - [ ] Optimize imports
- [ ] **Run Verification Procedure** after refactoring:
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run: `npx eslint --fix {refactored-files}`
    - [ ] Run: `npx prettier --write {refactored-files}`
    - [ ] Run: `npx vitest related run {refactored-files}`
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.2 files (artwork queries, forms, pages, tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/lib/db/admin/artwork.ts src/lib/validation/artwork.ts src/components/admin/ArtworkForm.tsx src/app/admin/artwork/`
    - [ ] Run:
          `npx prettier --write src/lib/db/admin/artwork.ts src/lib/validation/artwork.ts src/components/admin/ src/app/admin/artwork/ __tests__/lib/db/admin/artwork.test.ts __tests__/components/admin/ArtworkForm.test.tsx __tests__/app/admin/artwork/`
    - [ ] Run:
          `npm test -- __tests__/lib/db/admin/artwork.test.ts __tests__/components/admin/ArtworkForm.test.tsx __tests__/app/admin/artwork/`
    - [ ] Verify all checks pass before proceeding to Phase 3.3
- [ ] Manual testing:
    - [ ] Navigate to `/admin/artwork`
    - [ ] Create new artwork with all fields
    - [ ] Edit existing artwork
    - [ ] Delete artwork (verify confirmation)
    - [ ] Verify form validation (required fields, slug format)
...
**Goal**: Build image upload component with Sharp-based optimization and
...
- [ ] Review code quality:
    - [ ] Remove duplication in image processing code
...
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.3 files (image utils, upload API, ImageUploader
          component, tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/lib/utils/image.ts src/app/api/admin/upload/ src/components/admin/ImageUploader.tsx`
    - [ ] Run:
          `npx prettier --write src/lib/utils/image.ts src/app/api/admin/upload/ src/components/admin/ImageUploader.tsx __tests__/lib/utils/image.test.ts __tests__/app/api/admin/upload/ __tests__/components/admin/ImageUploader.test.tsx`
    - [ ] Run:
          `npm test -- __tests__/lib/utils/image.test.ts __tests__/app/api/admin/upload/ __tests__/components/admin/ImageUploader.test.tsx`
    - [ ] Verify all checks pass before proceeding to Phase 3.4
- [ ] Manual testing:
    - [ ] Upload image in create artwork form
    - [ ] Verify 3 variants created in Supabase Storage (check dashboard)
    - [ ] Verify URLs stored in database
    - [ ] Test file type validation (try uploading .txt file)
    - [ ] Test file size validation (try uploading >10MB image)
...
- [ ] Write tests:
    - [ ] Renders table with columns: Order Number, Customer, Date, Total, Status,
          Actions
...
- [ ] Table with columns: Order Number, Customer Name, Date, Total Amount,
      Status, Actions
...
- [ ] Review code quality:
    - [ ] Simplify order status update logic
...
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.4 files (order queries, OrdersList component, pages,
          tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/lib/db/admin/orders.ts src/components/admin/OrdersList.tsx src/app/admin/orders/`
    - [ ] Run:
          `npx prettier --write src/lib/db/admin/orders.ts src/components/admin/OrdersList.tsx src/app/admin/orders/ __tests__/lib/db/admin/orders.test.ts __tests__/components/admin/OrdersList.test.tsx __tests__/app/admin/orders/`
    - [ ] Run:
          `npm test -- __tests__/lib/db/admin/orders.test.ts __tests__/components/admin/OrdersList.test.tsx __tests__/app/admin/orders/`
    - [ ] Verify all checks pass before proceeding to Phase 3.5
- [ ] Manual testing:
    - [ ] Navigate to `/admin/orders`
    - [ ] View order list with pagination
    - [ ] Filter by status (pending, paid, shipped, etc.)
    - [ ] Paginate through orders (if multiple pages)
    - [ ] Click order to view details
    - [ ] Update order status (verify dropdown works)
    - [ ] Add admin note (verify saves)
    - [ ] Add tracking number (verify saves)
...
**Goal**: Build CRUD interfaces for projects and events (similar to artwork
management).
...
- [ ] Create `__tests__/app/admin/projects/new/page.test.tsx` (create page
      tests)
- [ ] Create `src/app/admin/projects/new/page.tsx` (create page)
- [ ] Create `__tests__/app/admin/projects/[id]/edit/page.test.tsx` (edit page
      tests)
...
- [ ] Review code quality across projects and events:
    - [ ] Look for duplication between ProjectForm and EventForm
...
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.5 files (project/event queries, forms, pages,
          validation schemas, tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/lib/db/admin/projects.ts src/lib/db/admin/events.ts src/lib/validation/project.ts src/lib/validation/event.ts src/components/admin/ProjectForm.tsx src/components/admin/EventForm.tsx src/app/admin/projects/ src/app/admin/events/`
    - [ ] Run:
          `npx prettier --write src/lib/db/admin/projects.ts src/lib/db/admin/events.ts src/lib/validation/ src/components/admin/*Form.tsx src/app/admin/projects/ src/app/admin/events/ __tests__/`
    - [ ] Run:
          `npm test -- __tests__/lib/db/admin/projects.test.ts __tests__/lib/db/admin/events.test.ts __tests__/components/admin/ProjectForm.test.tsx __tests__/components/admin/EventForm.test.tsx __tests__/app/admin/projects/ __tests__/app/admin/events/`
    - [ ] Verify all checks pass before proceeding to Phase 3.6
- [ ] Manual testing:
    - [ ] **Projects**: Create/edit/delete projects
    - [ ] **Projects**: Upload image, verify 3 variants
    - [ ] **Projects**: Test validation (required fields, dates)
    - [ ] **Events**: Create/edit/delete events
    - [ ] **Events**: Upload image, verify 3 variants
    - [ ] **Events**: Test validation (start date required, URL format)
    - [ ] Verify changes appear on public `/in-the-works` page
...
- [ ] Review revalidation logic:
    - [ ] Ensure all mutations trigger revalidation
    - [ ] Check for missing revalidation calls
    - [ ] Optimize paths (avoid over-revalidation)
- [ ] **Run Verification Procedure** after refactoring:
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run: `npx eslint --fix {refactored-files}`
    - [ ] Run: `npx prettier --write {refactored-files}`
...
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.6 files (revalidate API route, updated action files,
          tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/app/api/admin/revalidate/ src/app/admin/artwork/actions.ts src/app/admin/projects/actions.ts src/app/admin/events/actions.ts`
    - [ ] Run:
          `npx prettier --write src/app/api/admin/revalidate/ src/app/admin/*/actions.ts __tests__/app/api/admin/revalidate/`
    - [ ] Run: `npm test -- __tests__/app/api/admin/revalidate/`
    - [ ] Verify all checks pass before proceeding to Phase 3.7
- [ ] Manual testing:
    - [ ] **Artwork**: Create artwork → verify appears on `/gallery` immediately
    - [ ] **Artwork**: Update artwork → verify changes on `/gallery/[slug]`
    - [ ] **Artwork**: Delete artwork → verify disappears from `/gallery`
    - [ ] **Projects**: Create project → verify on `/in-the-works`
    - [ ] **Events**: Create event → verify on `/in-the-works`
    - [ ] Open public pages in incognito/private browsing to verify cache cleared
...
- [ ] Write tests for:
    ...
    - [ ] `getAdminById(id)`
    - [ ] `createAdmin(data)` - create new admin user (also creates Supabase Auth
          user)
...
- [ ] Implement query functions:
    - [ ] `getAllAdmins()`
    - [ ] `getAdminById(id)`
    - [ ] `createAdmin(data)` - **Important**: Must also create Supabase Auth
          user:
...
- [ ] Review code quality:
    - [ ] Ensure proper role checking throughout
    - [ ] Improve admin user management UX
    - [ ] Add JSDoc comments
    - [ ] Verify security (cannot deactivate self)
- [ ] **Run Verification Procedure** after refactoring:
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run: `npx eslint --fix {refactored-files}`
    - [ ] Run: `npx prettier --write {refactored-files}`
...
- [ ] Run full test suite: `npm test`
- [ ] **End-of-Phase Verification**:
    - [ ] List ALL Phase 3.7 files (administrator queries, AdminForm, settings
          page, tests)
    - [ ] Run: `npx tsc --noEmit`
    - [ ] Run:
          `npx eslint --fix src/lib/db/admin/administrators.ts src/components/admin/AdminForm.tsx src/app/admin/settings/`
    - [ ] Run:
          `npx prettier --write src/lib/db/admin/administrators.ts src/components/admin/AdminForm.tsx src/app/admin/settings/ __tests__/lib/db/admin/administrators.test.ts __tests__/components/admin/AdminForm.test.tsx __tests__/app/admin/settings/`
    - [ ] Run:
          `npm test -- __tests__/lib/db/admin/administrators.test.ts __tests__/components/admin/AdminForm.test.tsx __tests__/app/admin/settings/`
    - [ ] Verify all checks pass before proceeding to Phase 3.8
- [ ] Manual testing:
    - [ ] **As super_admin**: Login and navigate to `/admin/settings`
    - [ ] **As super_admin**: View admin users list
    - [ ] **As super_admin**: Add new admin user (verify Supabase Auth user
          created)
    - [ ] **As super_admin**: Edit existing admin (change name/role)
    - [ ] **As super_admin**: Deactivate admin (verify cannot deactivate self)
    - [ ] **As regular admin**: Try accessing `/admin/settings` (should redirect
          or show access denied)
    - [ ] Logout as super_admin, login as new admin (verify works)

---

### Phase 3.8: Final Testing & Polish

**Goal**: Ensure comprehensive test coverage, handle edge cases, and polish the
admin experience.
...
#### 3.8.11: Final Code Verification

**Complete End-of-Phase-3 Verification**

- [ ] **Comprehensive Verification** for ALL of Phase 3:
    - [ ] List EVERY file created/modified across all Phase 3 sub-phases (3.1-3.8)
    - [ ] Run full TypeScript check: `npx tsc --noEmit`
    - [ ] Run full ESLint check and fix: `npm run lint` (or
          `npx eslint --fix src/ __tests__/`)
    - [ ] Run full Prettier format: `npm run format` (or
          `npx prettier --write src/ __tests__/`)
    - [ ] Run complete test suite: `npm test`
    - [ ] Verify test coverage: `npm run test:coverage`
        - Admin queries: 100% coverage
        - Admin components: 80-85% coverage
        - Admin pages: 70%+ coverage
        - API routes: 90%+ coverage
    - [ ] Fix any failures (max 3 attempts per issue)
    - [ ] If issues persist, document and request guidance
- [ ] **Build verification**:
...
- [ ] **Commit all changes** (only after all verifications pass):
    - [ ] Stage changes: `git add .`
    - [ ] Commit with clear message describing Phase 3 completion
    - [ ] Note: Pre-commit hooks will run automatically (lint-staged + TypeScript
          check)
...
The following features are **not** included in Phase 3 and may be added in
future phases:
...
- **Rich text editor**: WYSIWYG editor for descriptions (using plain textarea
  for MVP)
...
**Next Phase**:
[Phase 4: Shopping Cart & Checkout](./2025-10-25T17-55-00-mvp-implementation-plan.md#phase-4-shopping-cart--checkout-week-5)
...
export default async function OrdersPage({
    ...
    const { data: orders, error } = await getAllOrders(limit, offset, {
        status,
    });
