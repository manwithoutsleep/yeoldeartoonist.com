...
describe('useAuth Hook', () => {
	...
	describe('SignIn', () => {
		...
		describe('Failure Cases - Exception', () => {
			it('should handle exception thrown during sign in', async () => {
				...
				expect(signInResult.data).toBeNull();
				expect(signInResult.error).toEqual({ message: 'Network error' });
			});
			...
			it('should normalize error and set error state on exception', async () => {
				...
				expect(result.current.error).toEqual({ message: 'Connection timeout' });
			});
		});
		...
		describe('Error Clearing', () => {
			it('should clear previous error on new sign in attempt', async () => {
				...
				await act(async () => {
					await result.current.signIn('user@example.com', 'correctpassword');
				});
	...
	describe('SignUp', () => {
		describe('Success Cases', () => {
			...
			it('should call signUp with correct email and password', async () => {
				...
				await act(async () => {
					await result.current.signUp('test@example.com', 'secret123');
				});
		...
		describe('Failure Cases - Supabase Error', () => {
			it('should handle sign up error from Supabase', async () => {
				...
				expect(signUpResult.error).toEqual({ message: 'User already exists' });
			});
	...
	describe('SignOut', () => {
		...
		describe('Failure Cases - Exception', () => {
			it('should handle exception thrown during sign out', async () => {
				...
				expect(signOutResult.error).toEqual({ message: 'Network error' });
			});
			...
			it('should normalize error and set error state on exception', async () => {
				...
				expect(result.current.error).toEqual({ message: 'Connection lost' });
			});
		});
	...
	describe('Error Normalization', () => {
		it('should normalize Error instance', async () => {
			...
			let signInResult: any;
			await act(async () => {
				signInResult = await result.current.signIn('user@example.com', 'pass');
			});
		...
		it('should normalize object with message property', async () => {
			...
			expect(signInResult.error).toEqual({ message: 'Custom error message' });
		});
