...
describe("Artwork Database Queries - Integration Tests", () => {
    ...
    skipIfNoSupabase(
        "Supabase integration tests (requires connection)",
        () => {
            describe("error structure validation", () => {
                ...
                it("error messages should be user-friendly strings", async () => {
                    ...
                    if (result.error) {
                        ...
                        ).not.toMatch(/sql|postgres|query/i);
                    }
                });
            });
            ...
            describe("environment-based error details", () => {
                ...
                it("should include details field in development mode for thrown errors", async () => {
                    process.env.NODE_ENV = "development";

                    // Call with nonexistent slug - if this throws an error, details should be included
                    const result = await getArtworkBySlug(
                        "definitely-nonexistent-slug",
                    );

                    // If we get an error in development, it should have details (if it's a caught exception)
                    if (result.error && result.error.code === "fetch_error") {
                        // This is a caught exception, so it should have details in development
                        expect(result.error).toHaveProperty("details");
                    }
                    // If it's a Supabase error (data not found), details may not be present
                });

                it("should not include details field in production mode", async () => {
                    ...
                    // If we get an error in production, it should NOT have details
                    if (result.error) {
                        expect((result.error as any).details).toBeUndefined();
                    }
                });

                it("error structure should be consistent across all functions in development", async () => {
                    process.env.NODE_ENV = "development";
