...
- ✅ Admin authentication and middleware with 15-minute session caching
...
#### Update Middleware to Include Admin Name

The middleware currently stores `userId`, `adminId`, `role`, and `expiresAt` in the session cookie, but the admin header needs to display the admin's name. Update the middleware:
...
#### Install Form Management Dependencies
...
- Middleware protection for `/admin/*` routes (except `/admin/login`)
...
2. **Centralized authorization**: All admin authorization happens in one place (middleware), not scattered across multiple RLS policies per table. Easier to audit and maintain.
...
2. **Middleware protection**: All admin routes protected by middleware that validates admin session before allowing access
...
6. **Defense in depth**: Middleware + API route checks + server-side enforcement = multiple layers of protection
...
- Client-side check for UI visibility (middleware enforces server-side)
...
Middleware stores `role` in session cookie for client-side checks.
...
**Critical**: Middleware protects `/admin/*` page routes but **does NOT protect `/api/admin/*` API routes**. All API routes must manually verify authentication.
...
**Security Note**: Client-side cookie checks are for UI convenience only. Always verify authorization server-side (middleware, Server Actions, or API routes).
...
- [ ] POST handler:
    - [ ] Verify admin session (check cookie or use middleware)
...
- [ ] POST handler:
    - [ ] Verify admin session (middleware or manual check)
