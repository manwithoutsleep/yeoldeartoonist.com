...
- [x] Add tests to `__tests__/lib/db/admin/dashboard.test.ts` for `getRecentOrders()`:
...
- [x] Add `getRecentOrders()` to `src/lib/db/admin/dashboard.ts`:
    ...
    - [x] Select relevant fields (order_number, customer_name, total_amount, status, created_at)
...
- [x] Write tests:
    ...
    - [x] Desktop: Shows all navigation links (Dashboard, Artwork, Orders, Projects, Events)
...
- [x] Lint and format: `npm run lint && npm run format`
